...
Transcription history is enabled by default (`history_enabled`, not exposed as a Settings toggle). Entries are saved to local history storage and cleaned up according to the History retention settings. Failed transcriptions are stored in `failed_audio/` with WAV audio and JSON metadata; the storage keeps up to three entries and the History page can retry or dismiss them.

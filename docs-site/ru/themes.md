...
Приложение копирует встроенные темы в каталог пользовательских тем при запуске. Отсутствующие темы копируются; существующие копии встроенных тем старого формата (не v2) перезаписываются на версию v2; существующие пользовательские темы формата v2 сохраняются (даже если они временно недействительны). Произвольные пользовательские папки invalid/non-v2 сканер пропускает, а не мигрирует. Если правки не видны, скопируйте обновлённые `theme.js` и `theme.json` в пользовательский каталог тем и перезагрузите/выберите тему снова.

Встроенные темы, которые сейчас собирает `bun run build:themes`, включают Default, Winamp Classic, Neon, Handy Pill, Metaballs, Metaballs 2.5D/3D, Lava Lamp, Living/Quiet Reed, Radiolarian, Paramecium Solo, Vorticella Bloom и aquarium/drift-темы.
